...
for (const file of files) {
  ...
  if (fm && /^draft:\s*true$/m.test(fm[1])) {
    ...
    // Same harvest for screenshots a component pulls in via an import, which
    // never use ![]() either. See the live-page version of this loop below.
  ...
  // This site swizzles DocItem/Content to render the frontmatter title as the
  // page heading, so a body H1 renders a second title. See STYLE_GUIDE.md
  // section 0. docs-vela requires exactly one H1; here the rule is inverted.
  const h1s = [...headingSource.matchAll(/^# (.+)$/gm)].map((m) => m[1].trim());
  if (h1s.length > 0)
    err(rel, `H1 "${h1s[0]}" in the body: the theme already renders the title`);
  ...
  // appeared on nearly every page and carried no information.
  const SCAFFOLD = [
    "Why this matters",
