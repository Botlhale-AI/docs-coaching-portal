...
| **Initiation Score** | Both | The result of the first attempt, kept separately from the latest result |
...
**What to look for:** **Complete** means finished, not passed. A course closes as complete when an agent uses their last retake, whatever they scored, so read it together with **Score**. The distance between **Initiation Score** and **Score** is what the course taught: a wide gap means the material worked on a second reading, and two identical figures mean the retake changed nothing, which points at the material rather than the agent. **Date Assigned** is the field that confirms a cycle ran, so a course with no rows carrying one has reached nobody yet. Sorting on **Due Date** brings the overdue to the top, which is a shorter list to work from than the whole page.
